# Jido Integration Workspace Persistence

## Scope

Jido Integration Workspace owns connector, auth, control-plane, and durable store orientation. This document is package-local and is the persistence contract for `.` in `jido_integration`.
...
In-memory auth and control-plane stores by default; local restart-safe and Postgres stores are explicit packages.
...
```bash
mix mr.pg.preflight when Postgres-backed acceptance is required; mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :local_restart_safe for file-backed local stores; :integration_postgres for canonical Postgres stores after capability preflight.
...
core/store_postgres owns migrations; core/store_local owns local file layout.
