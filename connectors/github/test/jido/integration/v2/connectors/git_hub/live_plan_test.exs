defmodule Jido.Integration.V2.Connectors.GitHub.LivePlanTest do
  ...
  test "prefers the explicit read issue number for combined all mode" do
    spec = %{
      repo: "agentjido/jido_integration_v2",
      write_repo: "agentjido/jido_integration_v2_sandbox",
      read_issue_number: 42
    }

    assert {:existing, target} = LivePlan.all_read_target(spec, [])
    assert target.repo == spec.repo
    assert target.issue_number == 42
    assert target.source == :explicit_issue_number
  end
