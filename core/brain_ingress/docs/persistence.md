# Jido Integration Brain Ingress Persistence

## Scope

Jido Integration Brain Ingress owns durable brain-to-lower-gateway submission acceptance and rejection owner. This document is package-local and is the persistence contract for `core/brain_ingress` in `jido_integration`.
...
Memory by default through selected control-plane/submission ledger behavior; local and Postgres ledgers are explicit.
...
```bash
cd core/brain_ingress && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :local_restart_safe or :integration_postgres when the ledger adapter and capability are passed explicitly.
...
Postgres submission-ledger migrations live in core/store_postgres.
