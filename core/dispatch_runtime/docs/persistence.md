# Jido Integration Dispatch Runtime Persistence

## Scope

Jido Integration Dispatch Runtime owns file-backed async dispatch transport state owner above control-plane truth. This document is package-local and is the persistence contract for `core/dispatch_runtime` in `jido_integration`.
...
File-backed transport records when configured; control-plane truth remains delegated.
...
```bash
cd core/dispatch_runtime && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :local_restart_safe or :integration_postgres only when paired with durable control-plane stores.
...
File layout is package-owned; Postgres truth stays in store_postgres.
