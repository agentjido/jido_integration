# Jido Integration Platform Persistence

## Scope

Jido Integration Platform owns public facade over auth, control-plane, review, target, lower-facts, and brain-ingress seams. This document is package-local and is the persistence contract for `core/platform` in `jido_integration`.
...
Delegates to auth/control-plane selected stores; facade itself owns no Repo.
...
```bash
cd core/platform && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :local_restart_safe or :integration_postgres only through delegated store capabilities.
...
Durable migration ownership stays in store packages.
