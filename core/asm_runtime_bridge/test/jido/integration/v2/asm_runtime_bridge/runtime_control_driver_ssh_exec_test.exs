defmodule Jido.Integration.V2.AsmRuntimeBridge.RuntimeControlDriverSSHExecTest do
  ...
  test "stream_run/3 proves ssh_exec through the unchanged runtime-control driver seam" do
    ...
    assert {:ok, %RunHandle{} = run, stream} =
             RuntimeControlDriver.stream_run(session, request, cli_path: cli_path)
  ...
  defp temp_dir!(prefix) do
    dir =
      Path.join(
        System.tmp_dir!(),
        "asm_runtime_bridge_ssh_exec_#{prefix}_#{System.unique_integer([:positive])}"
      )
