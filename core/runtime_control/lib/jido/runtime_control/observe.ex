defmodule Jido.RuntimeControl.Observe do
  @moduledoc """
  Runtime Control observability boundary for canonical telemetry events and spans.
  """
  ...
  def workspace(event), do: [:jido, :runtime_control, :workspace, event]

  @doc "Builds a canonical runtime telemetry event path."
  @spec runtime(atom()) :: event_name()
  def runtime(event), do: [:jido, :runtime_control, :runtime, event]

  @doc "Builds a canonical provider telemetry event path."
  @spec provider(atom()) :: event_name()
  def provider(event), do: [:jido, :runtime_control, :provider, event]

  @doc "Emits a runtime-control telemetry event with required metadata defaults."
  ...
  @doc "Starts a runtime-control telemetry span."
  ...
  @doc "Finishes a runtime-control telemetry span."
  ...
  @doc "Finishes a runtime-control span with exception metadata."
