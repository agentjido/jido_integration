defmodule Jido.Integration.V2.StorePostgres.Schemas.MemoryGovernedRecord do
  @moduledoc false
  ...
  schema "memory_governed" do
    ...
    field(:promotion_decision_ref, :string)
    field(:promotion_policy_ref, :string)
    field(:rebuild_spec, :map)
    field(:derived_state_attachment_ref, :string)
  ...
  def changeset(record, attrs) do
    ...
    |> unique_constraint(:fragment_id, name: :memory_governed_pkey)
    |> check_constraint(:evidence_refs, name: :memory_governed_evidence_refs_non_empty_check)
    |> check_constraint(:governance_refs,
      name: :memory_governed_governance_refs_non_empty_check
    )
    |> check_constraint(:embedding_dimension,
      name: :memory_governed_embedding_dimension_matches_vector_check
    )
    |> check_constraint(:embedding_model_ref,
      name: :memory_governed_embedding_model_dimension_pair_check
    )
    |> check_constraint(:source_node_ref, name: :memory_governed_source_node_ref_non_empty_check)
  end
