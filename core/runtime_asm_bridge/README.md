...
Integration-owned `Jido.Harness.RuntimeDriver` for the authored `asm` driver,
backed by `agent_session_manager` and the `cli_subprocess_core` lane beneath
it.
...
- localize the `agent_session_manager` dependency so connector packages can
  keep their shared dependency surface at `jido_harness`
