# Jido Model Provider Registry Persistence

## Scope

Jido Model Provider Registry owns governed model, provider, endpoint, capability, and pool ref owner. This document is package-local and is the persistence contract for `core/model_provider_registry` in `jido_integration`.
...
Ref-only memory evidence by default.
...
```bash
cd core/model_provider_registry && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable persistence profile preflight is required before any retained provider registry evidence claim.
...
No provider SDK or durable store is selected by default.
