defmodule Jido.Integration.Dispatch.RunStore.ETS do
  @moduledoc """
  ETS-backed run-record store for explicit local development mode.
  """
  ...
  @impl GenServer
  def init(_opts) do
    {:ok, %{table: :ets.new(:dispatch_run_store, [:set, :private])}}
  end

  @impl GenServer
  def handle_call({:put, %Run{} = run}, _from, state) do
    case conflicting_run_id(state.table, run) do
      nil ->
        :ets.insert(state.table, {run.run_id, run})
        {:reply, :ok, state}
  ...
  def handle_call({:list, opts}, _from, state) do
    runs = state.table |> :ets.tab2list() |> Enum.map(&elem(&1, 1))
    {:reply, filter_runs(runs, opts), state}
  end
  ...
  defp conflicting_run_id(table, %Run{} = run) do
    table
    |> :ets.tab2list()
    |> Enum.map(&elem(&1, 1))
