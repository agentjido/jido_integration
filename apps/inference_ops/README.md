...
`Jido.Integration.V2.invoke_inference/2` facade into three permanent proof
flows:
...
self-hosted route through `self_hosted_inference_core`, `llama_cpp_ex`, and
`req_llm` without reclaiming runtime ownership inside the app layer.
...
self-hosted lane uses the shared fake `llama-server` fixture script published
by `llama_cpp_ex`. The CLI lane uses a fake ASM backend under the real
