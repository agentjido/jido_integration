defmodule Jido.BoundaryBridge.AllocateBoundaryRequest do
  @moduledoc """
  Typed allocate request for the lower-boundary bridge.

  `allocation_ttl_ms` carries the startup TTL hint that lets `jido_os` reap an
  unattached boundary if the caller crashes before the kernel claims it.
  """
  ...
  @schema Zoi.struct(
            ...
            %{
              boundary_session_id:
                Contracts.non_empty_string_schema("allocate_boundary_request.boundary_session_id"),
              backend_kind: Contracts.atomish_schema("allocate_boundary_request.backend_kind"),
              boundary_class:
                Contracts.atomish_schema("allocate_boundary_request.boundary_class")
                |> Zoi.nullish()
                |> Zoi.optional(),
       ...
       | attach: AttachConfig.new!(request.attach),
         ...
         allocation_ttl_ms:
           validate_positive_integer!(
             request.allocation_ttl_ms,
             "allocate_boundary_request.allocation_ttl_ms"
           ),
         readiness_timeout_ms:
           validate_positive_integer!(
             request.readiness_timeout_ms,
             "allocate_boundary_request.readiness_timeout_ms"
           )
     }}
  rescue
