defmodule Jido.Integration.V2.Connectors.GitHub.InstallBinding do
  @moduledoc false
  ...
  @spec from_personal_access_token(String.t()) :: t()
  def from_personal_access_token(token) when is_binary(token) do
    token = String.trim(token)

    if token == "" do
      raise ArgumentError, "GitHub personal access token must not be empty"
    end

    build_binding("personal_access_token", %{access_token: token}, nil)
  end
  ...
  defp auth_profile(profile_id) do
    ...
    |> case do
      nil ->
        raise "GitHub auth profile #{inspect(profile_id)} is required for install binding"
